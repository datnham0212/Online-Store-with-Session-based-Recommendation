...
('elu_param', 0.5),  # Tham số cho hàm kích hoạt ELU
('layers', [224]),  # Cấu trúc mạng với một lớp ẩn có 224 đơn vị
('n_epochs', 10),  # Số lượng epoch để huấn luyện
('batch_size', 80),  # Kích thước batch trong quá trình huấn luyện
('dropout_p_embed', 0.5),  # Xác suất dropout cho embedding
('dropout_p_hidden', 0.05),  # Xác suất dropout cho các lớp ẩn
...
('sample_alpha', 0.4),  # Tham số alpha cho sampling
('bpreg', 1.95),  # Hệ số điều chỉnh cho bpr-max
('logq', 0.0)  # Giá trị logq (có thể liên quan đến sampling hoặc regularization)
])
