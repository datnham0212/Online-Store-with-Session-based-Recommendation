...
('layers', [480]),  # Cấu trúc mạng với một lớp ẩn có 480 đơn vị
('n_epochs', 10),  # Số lượng epoch để huấn luyện
('batch_size', 48),  # Kích thước batch trong quá trình huấn luyện
('dropout_p_embed', 0.0),  # Xác suất dropout cho embedding
('dropout_p_hidden', 0.2),  # Xác suất dropout cho các lớp ẩn
('learning_rate', 0.07),  # Tốc độ học (learning rate)
('momentum', 0.0),  # Hệ số momentum cho thuật toán tối ưu
...
('logq', 1.0)  # Giá trị logq (có thể liên quan đến sampling hoặc regularization)
])
